...
export default function Step5Review({ draft, onBack, onSubmit, submitting, error }: Props) {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Payments */}
      <section className="flex flex-col gap-3">
        <h3 className="text-xs font-bold uppercase tracking-wide text-gray-400">Payment Options</h3>
        <div className="bg-gray-50 rounded-xl p-4 flex flex-wrap gap-2">
          {draft.accepts_qr && <span className="px-3 py-1 rounded-full bg-blue-100 text-xs text-blue-700 font-medium">QR Code</span>}
          {draft.accepts_cash && <span className="px-3 py-1 rounded-full bg-green-100 text-xs text-green-700 font-medium">Pay at Venue</span>}
          {draft.accepts_gateway && <span className="px-3 py-1 rounded-full bg-purple-100 text-xs text-purple-700 font-medium">Online Gateway</span>}
        </div>
      </section>
